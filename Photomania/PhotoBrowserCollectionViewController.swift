...
class PhotoBrowserCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
  ...
  override func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
    ...
    if let image = self.imageCache.objectForKey(imageURL) as? UIImage {
        ...
        cell.request = Alamofire.request(.GET, imageURL).validate(contentType:["image/*"]).responseImage({ (request, _, image, error) -> Void in
  ...
  override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
    ...
    func populatePhotos() {
        ...
        Alamofire.request(Five100px.Router.PopularPhotos(self.currentPage)).responseJSON(options: NSJSONReadingOptions()) { (_, _, json, error) -> Void in
            if error == nil {
