...
class PhotoViewerViewController: UIViewController, UIScrollViewDelegate, UIPopoverPresentationControllerDelegate, UIActionSheetDelegate {
  ...
  override func viewDidLoad() {
    ...
    func loadPhoto() {
        Alamofire.request(Five100px.Router.PhotoInfo(self.photoID, .Large)).validate().responseObject { (_, _, photoInfo: PhotoInfo?, error) -> Void in
            if error == nil {
                ...
                Alamofire.request(.GET, photoInfo!.url).validate().responseImage({ (_, _, image, error) -> Void in
    ...
    func downloadPhoto() {
        Alamofire.request(Five100px.Router.PhotoInfo(photoInfo!.id, .XLarge)).validate().responseObject() {
            ...
            if error == nil && photoInfo != nil {
                ...
                Alamofire.download(.GET, imageURL, destination: destination).progress(closure: { (_, totalBytesRead, totalBytesExpectedToRead) -> Void in
                    dispatch_async(dispatch_get_main_queue(), { () -> Void in
